...
position pos;
void wherexy() { pos.x = wherex(); pos.y = wherey(); }
...
void cvmCenterTxt(std::string texte, unsigned int Temps, cvmColor couleur)
{
	...
	if ( texte.size() <= scrWidth )
		std::cout << std::setw ( (scrWidth - texte.size() ) / 2 ) << std::left << "";
...
void cvmDefilTxt(std::string texte, unsigned int Temps, cvmColor couleur)
{
	cvmSetColor(couleur);
	for (unsigned int i(0); i<texte.size(); i++) //Défiler
	{
